...
> **2026-09-26 认证架构决策：**每次注册校邮收码，用户设置私有用户名和独立密码；以后用用户名和密码登录，不再通过邮箱查找旧号。同一精确邮箱地址同时最多一个有效账号，注销后释放配额，再次收码建立全新账号。密码找回需事先保存的独立凭据，邮箱验证码不能单独重置。两方通过随机资格槽位协调配额；串通或共同泄漏时能连接邮箱与账号。详情及未完成的独立安全评审见[认证与隐私架构决策](auth-privacy-architecture-decision.md)。下方按日期记载的邮箱唯一登录内容为历史进度，本轮没有改生产代码。
...
- 2026-09-26 暂停继续实现认证主链，转入架构重评：需要解决唯一邮箱验证方主动恶意时的既有账号接管风险，同时保持 To C 用户体验。最终方案允许推翻“邮箱验证码是唯一登录凭证”，候选包括匿名校园资格凭证 + Passkey/WebAuthn、学校官方 SSO + 匿名层、原协议强化及 ZK 类证明。决策任务与 Codex 启动提示见 `docs/design/auth-privacy-gpt6-handoff.md`；在用户确认 GPT-6 结论前不得把任何候选写成已定实现。
